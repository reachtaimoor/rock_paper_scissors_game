...
model = load_model('rps_model.h5')  # Ensure you have a trained model saved as 'rps_model.h5'
...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        # Crop the hand region from the masked frame
        hand_image = cv2.resize(masked_frame, (300, 300))  # Resize to the model's input size
        hand_image_normalized = hand_image / 255.0  # Normalize to [0, 1]
